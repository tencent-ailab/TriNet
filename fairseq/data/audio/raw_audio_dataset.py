...
from fairseq.data.audio.audio_utils import (is_sf_audio_data, parse_path, read_from_stored_zip)
...
class RawAudioDataset(FairseqDataset):
    def __init__(
        ...
        if self.compute_mask_indices:
            ...
            self._feat_len = mask_compute_kwargs["encoder_embed_dim"]
    ...
    def _compute_mask_indices(self, dims, padding_mask):
        batch_len, seq_len, feat_len = dims
        ...
        if self.mask_compute_kwargs["mask_channel_prob"] > 0:
            ...
            mask_channel_indices = (
                torch.from_numpy(mask_channel_indices).unsqueeze(1).expand(-1, seq_len, -1)
            )
    ...
    def collater(self, samples):
        ...
        collated_sources = sources[0].new_zeros(len(sources), target_size, self.input_dim)

        # data_lens = torch.LongTensor([target_size] * len(sources))
        # print("data_lens:{}".format(' '.join(str(s) for s in data_lens)))

        padding_mask = (
            torch.BoolTensor(len(sources), target_size).fill_(False) if self.pad else None
        )
        for i, (source, size) in enumerate(zip(sources, sizes)):
            ...
            if diff == 0:
                ...
                collated_sources[i] = torch.cat(
                    [source, source.new_full((-diff, self.input_dim), 0.0)]
                )
                padding_mask[i, diff:] = True
            else:
                collated_sources[i] = self.crop_to_max_size(source, target_size)

        # print("padding_mask:{}".format(" ".join(str(s) for s in padding_mask[0])))

        # input = {"source": collated_sources, "data_lens":data_lens}
        ...
        if padding_mask is None:
            print("padding_mask is None in collater.")
        else:
            print("padding_mask is not None in collater.")
        """
...
class FileArkDataset(RawAudioDataset):
    def __init__(
            self,
            manifest_path,
            sample_rate,
        ...
        with open(manifest_path, "r") as f:
            ...
            for i, line in enumerate(f):
                ...
                if min_sample_size is not None and sz < min_sample_size:
                    ...
                    self.skipped_indices.add(i)
                    continue
                self.fnames.append(self.text_compressor.compress(items[1]))
                sizes.append(sz)
        logger.info(f"loaded {len(self.fnames)}, skipped {skipped} samples")
    ...
    def __getitem__(self, index):
        from fairseq.data.audio import kaldi_io

        fn = self.fnames[index]
        fn = fn if isinstance(self.fnames, list) else fn.as_py()
        fn = self.text_compressor.decompress(fn)

        ark = kaldi_io.read_mat(fn)
        feats = torch.from_numpy(ark).float()
        return {"id": index, "source": feats}

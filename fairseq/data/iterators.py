...
class EpochBatchIterator(EpochBatchIterating):
    ...
    def change_dataset(self,dataset,itr_update, collate_fn, batch_sampler, seed=1, num_shards=1, shard_id=0,num_workers=0, epoch=0, buffer_size=0,):
        assert isinstance(dataset, torch.utils.data.Dataset)
        self.dataset = dataset
        self.collate_fn = collate_fn
    ...
    def next_epoch_itr(
        self, shuffle=True, fix_batches_to_gpus=False, set_dataset_epoch=True, is_pretrain=True
    ):
        """Return a new iterator over the dataset.

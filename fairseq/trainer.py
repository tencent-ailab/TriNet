    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                # _adaptive_prune_heads functions to get the top X heads with most importance.
                ...
                # I use the argument --mha-heads-to-keep to prune the Roberta model into a pruned version
                # which matches the pruned ckpt.
                if (
                    ...
                    logger.info(
                        f"Prune model: keep {self.model.args.mha_heads_to_keep} heads"
                        f" for each multihead attention module"
                    )
                ...
                # Fine tune model (e.g. roberta encoder) on a certain datasets with regularization
                # After the model is trained. User could use _get_fc_rank and _prune_fc_layer
                # functions to get the top X units with most importance.
                # Then user uses the rank to prune a new roberta encoder and save the pruned ckpt manually.
                # User will fine tune the the new roberta encoder via the ckpt saved above
                # To get rid of registering different pruned version of Roberta,
                # I use the argument --ffn-blocks-to-remove to prune the Roberta model into a pruned version
                # which matches the pruned ckpt.
                if (
                        safe_hasattr(self.model, "args")
                    ...
                    logger.info(
                        f"Prune model: remove {self.model.args.ffn_blocks_to_remove} ffn blocks"
                        f" for each transformer layer"
                    )
                    for layer in self.model.encoder.sentence_encoder.layers:

...
class ModelCriterion(FairseqCriterion):
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        metrics.log_scalar("loss", loss_sum / sample_size, sample_size, round=9)
        ...
        for k in logging_outputs[0]:
            if k not in builtin_keys:
                ...
                if k.startswith("loss_"):
                    metrics.log_scalar(k, val / sample_size, sample_size, round=9)
                else:
                    metrics.log_scalar(k, val / world_size, round=9)
